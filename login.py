...
class main:
 ...
 # , relwidth=1, relheight=1)
    ...
    # Draw Widgets
    def widgets(self):
        self.head = tk.Label(self.master, text='Login', background='white', foreground="skyblue3", font=('Times', 40),
                             pady=20)
        self.head.pack()
        ...
        # self.head.pack()
        self.logf = tk.Frame(self.master, padx=50, pady=50, background='steelblue1')
        tk.Label(self.logf, text='Username ', background='steelblue1', foreground="white", font=("aerial", 15), pady=5,
                 padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.username, bd=5, background="white", font=('', 15)).grid(row=0, column=1)
        tk.Label(self.logf, text='Password ', background='steelblue1', foreground="white", font=("aerial", 15), pady=5,
                 padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.password, bd=5, background="white", font=('', 15), show='*').grid(row=1,
                                                                                                                column=1)
        tk.Button(self.logf, text=' Login ', command=self.login, bd=2, font=("Times New Roman", 20), background="steelblue3",
                  foreground="white", padx=4, pady=4).grid(rowspan=15,columnspan=15)
